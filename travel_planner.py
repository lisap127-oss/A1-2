...
def generate_report(recommendation: dict, restaurants: list,
    ...
    rest_str = "\n".join(rest_lines) if rest_lines else "  - 데이터 없음"
...
def save_results(recommendation: dict, restaurants: list,
    ...
    print(f"[저장] JSON : {filename}")

    # ✅ 추가: 리포트를 별도 .md 파일로 저장
    md_filename = Path(f"results/travel_{date}.md")
    with open(md_filename, "w", encoding="utf-8") as f:
        f.write(report)
    print(f"[저장] MD   : {md_filename}")
